export default {
	...
	settings: {
		...
		ssoStatus: {
			...
			unavailableDesc:
				'单点登录暂时无法启动。请切换到手动输入，使用坚果云 WebDAV 账号和应用密码继续同步。',
			unavailableNotice: '单点登录暂时无法启动，请在设置中切换到手动输入。',
			compatibility:
				'坚果云单点登录使用 Obsidian 的旧版坚果云回调路径。如果同时启用官方坚果云同步插件，可能只有其中一个插件能收到登录回调。',
		...
		ai: {
			...
			mcp: {
				...
				noUrl: '尚未设置 MCP endpoint',
				namePlaceholder: '名称',
				headersPlaceholder: 'Authorization: Bearer ...',
				local: {
					name: '本地 MCP Server',
					desc: '开启后，其他本机 MCP 客户端可以连接果札。优先用 stdio 配置；HTTP 地址适合支持 Streamable HTTP 的客户端。',
					port: '本地端口',
